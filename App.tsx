...
import { BotIcon, TerminalIcon, PlusIcon } from './components/Icons';
...
const App: React.FC = () => {
  ...
  const handleSendMessage = async (content: string, isRetry = false) => {
    ...
    if (addMatch) {
      ...
      setSessions(prev => prev.map(s => s.id === currentSessionId ? { ...s, messages: [...s.messages, userMsg, botMsg], updatedAt: new Date() } : s));
      return;
    }
  ...
  return (
    <div className="flex h-screen w-full overflow-hidden relative bg-[#0a0a0a]">
      <div className={`fixed top-0 left-0 w-full z-40 flex items-center justify-center p-6 transition-all duration-1000 ease-in-out ${
        hasMessages ? 'scale-90 translate-y-[-10px] opacity-60 pointer-events-none' : 'scale-100 translate-y-0 opacity-100'
      }`}>
        <div className="flex items-center gap-3 select-none">
          <h1 className="text-xl md:text-2xl font-bold tracking-[0.2em] uppercase text-white font-['JetBrains_Mono']">
      ...
      <button 
        ...
        className={`fixed top-6 left-6 z-50 p-3 rounded-full bg-white/5 border border-white/10 text-white/60 hover:text-white hover:bg-white/10 hover:border-white/20 transition-all duration-500 active:scale-95 group ${
          hasMessages ? 'opacity-100 translate-x-0' : 'opacity-0 -translate-x-10 pointer-events-none'
        }`}
        title="New Chat"
      >
        <PlusIcon className="w-5 h-5 transition-transform duration-500 group-hover:rotate-90" />
      </button>
      ...
      <main className="flex-1 flex flex-col relative z-10 overflow-hidden">
        ...
        <div className={`fixed bottom-0 left-0 w-full h-40 bg-gradient-to-t from-[#0a0a0a] via-[#0a0a0a]/90 to-transparent pointer-events-none transition-opacity duration-1000 z-20 ${hasMessages ? 'opacity-100' : 'opacity-0'}`} />
      </main>
